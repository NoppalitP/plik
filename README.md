...
|---|:---:|:---:|:---:|
| **การสลับภาษาอัตโนมัติ** | **ทันทีใน 0ms (CHA Bigram Engine)** | มี แต่ False Positive สูง (สลับคำทับศัพท์มั่ว) | ❌ ไม่มี (ต้องกดลบพิมพ์ใหม่เอง) |
| **Code Guard (คุ้มครองโค้ด & CLI)** | ✅ **มี (ละเว้นคำสั่ง Git, CLI, ไฟล์โค้ด)** | ❌ ไม่มี (แปลงโค้ดและคำสั่งเสียหาย) | ❌ ไม่มี |
| **แก้ไขคำผิดในตัว (Typo Engine)** | ✅ **มี (แก้ทั้งคำผิดไทย นะค่ะ->นะคะ และอังกฤษ)** | ❌ ไม่มี (สลับแค่ผังแป้นพิมพ์) | ❌ ไม่มี (มีแค่ Spellcheck ขีดเส้นแดง) |
| **สถาปัตยกรรมความปลอดภัย (Privacy)** | ✅ **100% Air-gapped Offline (Zero Socket)** | ⚠️ โปรแกรมเก่าออฟไลน์ / เครื่องมือใหม่ต่อ Cloud | ✅ ทำงานในเครื่อง (มี Telemetry OS) |
| **การใช้ทรัพยากร (RAM Working Set)** | ✅ **~33 MB (วัดจากการทำงานจริง)** | ⚠️ ~20 MB (Win32 เก่า) ถึง 150 MB (Electron) | ✅ รวมอยู่ในหน่วยความจำระบบ |
| **ความเข้ากันได้กับ Windows 11 64-bit** | ✅ **Native 64-bit Hook สมบูรณ์** | ⚠️ ปิดการพัฒนาไปแล้ว (32-bit มักหลุดบน Win11) | ✅ มาตรฐานระบบปฏิบัติการ |
| **Single-Instance Protection** | ✅ **Win32 Kernel Named Mutex ป้องกันเปิดซ้อน** | ❌ เช็ก FindWindow แบบเก่า (ค้างข้าม Session ได้) | — |
| **Instant Undo (ย้อนคืนค่าทันที)** | ✅ **กด Backspace 1 ครั้ง คืนค่าเดิมทันที** | ❌ ต้องกดปุ่มลัดสลับกลับ หรือลบพิมพ์ใหม่ | — |
| **ความโปร่งใส (Open Source)** | ✅ **MIT License ตรวจสอบโค้ดบน GitHub ได้** | ❌ Closed Source Freeware | ❌ Proprietary Closed Source |
